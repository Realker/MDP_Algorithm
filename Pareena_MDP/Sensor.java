...
public class Sensor {
	...
	public boolean sense_location(Map map, int x, int y, int robot_distance) {
		...
		if (x >= 0 && y >= 0 && x < map_width && y < map_height) {
			...
			if (map.simulated_map[y][x] == ExplorationTypes.convert_type_to_int("OBSTACLE")
					|| map.simulated_map[y][x] == ExplorationTypes.convert_type_to_int("NOT_OBSTACLE")) {
				score = -score;
				hit_wall = true;
			}
			map.set_ranking(x, y, score);
		} else
			hit_wall = true;
