...
public class Map {
	...
	public void set_nodes() { //for every node set obstacle is there is 
		for (int i = 0; i < map_height; i++) {
			for (int j = 0; j < map_width; j++) {
				...
				if (grid_map[i][j] != 0) { //if not 0 means there is an obstacle 
	...
	public void set_map_ranking() { //set bad areas in the map that the robot should avoid 
	...
	public void calculate_space() {
		...
		for (int i = 0; i < map_height; i++) {
			loop_column: for (int j = 0; j < map_width; j++) {
				node = node_map[i][j];
				node.set_num_spaces(0); //num of free space 

				if (node.is_obstacle()) { //if the node is obstacle set free space to 0
				...
				node.set_num_spaces(3); //else it means that the 3 by 3 grid around the node is empty 
			}
		}
	}
    ...
    public void reset_map(){ //reset map (simply specify that it is not empty)
        ...
        set_map_ranking(); //set bad areas in the map that the robot should avoid 
        update_map(); //update map with diff types of nodes + num of free spaces
    }
	...
	public void set_node_waypoint(int x, int y){ //3by3 grid around the node --> check if the node might be out of bounds
    ...
    public Map(int[][] map) {
		...
		set_nodes(); //set node obstacles
		set_neighbours(); //set node neighbours
		set_map_ranking(); //set bad areas in map that robot shld not traverse
	...
	public void set_negative_type(){ //intially was FPoptimize 
	...
	public void set_map() { //create an optimised map 
